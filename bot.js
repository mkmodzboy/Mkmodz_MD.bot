...
bot.onText(/\/pair(?:\s+(.+))?/, async (msg, match) => {
  ...
  // 🔥 GROUP MEIN /pair LIKHA TO SAME STYLISH MESSAGE
  if (isGroup) {
    return sendGroupMessage(chatId, msg.message_id);
  }

  // 🔥 PRIVATE CHAT MEIN DIRECT PAIRING PROCESS
  ...
  if (["252", "201"].includes(countryCode)) {
    return bot.sendMessage(
      chatId,
      '❌ *Numbers with this country code are not supported.*',
      { parse_mode: 'Markdown' }
    );
  }

  const pairingKey = text;
  if (pairingInProgress.has(pairingKey)) {
    return bot.sendMessage(chatId, '⏳ Pairing for this number is already in progress. Please wait a moment.');
  }
  pairingInProgress.set(pairingKey, Date.now());
  ...
  const pairedCount = files.filter(
    f => f.endsWith('@s.whatsapp.net')
  ).length;
  ...
  try {
    ...
    delete require.cache[
      require.resolve('./pair.js')
    ];

    pairingInProgress.delete(pairingKey);
...
bot.on('message', async (msg) => {
  ...
  if (/[a-z]/i.test(text)) {
    return bot.sendMessage(
      chatId,
      '❌ Letters are not allowed. Send only numbers.'
    );
  }
  ...
  if (pairedCount >= 1000) {
    return bot.sendMessage(
      chatId,
      '❌ Pairing limit reached. Try again later.'
    );
  }
  ...
  try {
...

      {
        ...
        reply_markup: {
          inline_keyboard: [
            [
              {
                text: `📋 Copy: ${cuObj.code}`,
                callback_data:
                  `copy_code_${cuObj.code}`
              }
            ]
          ]
        }
    ...
    bot.sendMessage(
      chatId,
      '❌ Pairing failed. Try again later.'
    );
  }
});
...
bot.onText(
  ...
  async (msg, match) => {
    ...
    if (isGroup) {
      return bot.sendMessage(
        chatId,
        '❌ Please use /unpair in my private chat.',
        { parse_mode: 'Markdown' }
      );
    }
    ...
    try {
      ...
      if (/[a-z]/i.test(input)) {
        return bot.sendMessage(
          chatId,
          'Letters not allowed. Use: /unpair 923xxxxxxxxx',
          { parse_mode: 'Markdown' }
        );
      }
      ...
      if (!/^\d{7,15}$/.test(input)) {
        return bot.sendMessage(
          chatId,
          'Invalid format. Use: /unpair 923xxxxxxxxx',
          { parse_mode: 'Markdown' }
        );
      }
      ...
      return bot.sendMessage(
        chatId,
        `✅ Paired user *${input}* has been deleted successfully`,
        { parse_mode: 'Markdown' }
      );
...
(async () => {
  ...
  console.log(
    '✅ Bot Username: @bot_hosting_v1_bot'
  );

  console.log(
    '✅ Features: /pair, /unpair, /start'
  );
