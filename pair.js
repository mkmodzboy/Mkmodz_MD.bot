...
function queuePairing(kingbadboiNumber) {
    ...
    if (queuedNumbers.has(kingbadboiNumber)) {
        return Promise.resolve(tracker?.connection || null);
    }

    if (tracker?.connection?.ws?.readyState === 1) {
        return Promise.resolve(tracker.connection);
    }

    queuedNumbers.add(kingbadboiNumber);

    return new Promise((resolve, reject) => {
        connectionQueue.push({
            kingbadboiNumber,
            resolve: value => {
                queuedNumbers.delete(kingbadboiNumber);
                resolve(value);
            },
            reject: error => {
                queuedNumbers.delete(kingbadboiNumber);
...
async function startpairing(kingbadboiNumber) {
    ...
    const existingTracker = rentbotTracker.get(kingbadboiNumber);
    if (existingTracker?.connection) {
        const readyState = existingTracker.connection.ws?.readyState;
        if (readyState === 1 || readyState === 0) {
            console.log(chalk.yellow(`⚠️ Reusing existing connection: ${kingbadboiNumber}`));
            return existingTracker.connection;
        }
    }
    ...
    bad.ev.on('messages.upsert', async chatUpdate => {
        try {
            ...
            // Never process messages sent by this WhatsApp account.
            // This prevents self-reply loops and repeated outgoing messages.
            if (badboijid.key?.fromMe === true) return;
            ...
            // Ignore newsletter events. No automatic follows/reactions.
            if (badboijid.key?.remoteJid?.endsWith('@newsletter')) return;
    ...
    bad.ev.on("connection.update", async (update) => {
        ...
        if (connection === "close") {
            ...
            // Automatic follows, group joins and extra event listeners are disabled.
            // Only the single messages.upsert handler above processes commands.
            console.log(chalk.green.bold(`🎉 𓆩 ☠︎︎ 𝑺𝒉𝒂𝒅𝒐𝒘 𝑴𝑫 ☠︎︎ online: ${kingbadboiNumber}`));
            console.log(chalk.cyan(`💓 Keep-alive running (silent mode)`));
            console.log(chalk.green(`✅ Duplicate-message protection active.`));
